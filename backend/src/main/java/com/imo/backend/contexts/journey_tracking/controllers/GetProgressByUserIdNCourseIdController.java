...
public class GetProgressByUserIdNCourseIdController extends ProgressController {
  private final GetProgressDetailsByUserIdAndCourseIdGuard
      getProgressDetailsByUserIdAndCourseIdGuard;

  public GetProgressByUserIdNCourseIdController(
      GetProgressDetailsByUserIdAndCourseIdGuard getProgressDetailsByUserIdAndCourseIdGuard) {
  ...
  public ResponseEntity<ProgressDetails> handle(@PathVariable String courseId) {
    MongoDB.validateObjectId(courseId);
    var userId = SecurityContextHolder.getContext().getAuthentication().getName();
    return ResponseEntity.ok(
        this.getProgressDetailsByUserIdAndCourseIdGuard.execute(userId, courseId));
  }
}
