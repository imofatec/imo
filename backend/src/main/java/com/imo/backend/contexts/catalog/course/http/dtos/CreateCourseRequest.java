...
public record CreateCourseRequest(
    @NotBlank(message = "Preencha o nome do curso")
        ...
        String name,
    @NotNull(message = "Preencha a categoria do curso") Categories category,
    @NotBlank(message = "Preencha o nível do curso") String level,
    @NotBlank(message = "Preencha a descrição do curso")
        @Size(
            min = 10,
            max = 300,
            message = "A descrição do curso precisa ter de 10 a 300 caracteres")
        String description,
    @Size(min = 1, max = 100, message = "Um curso pode ter no mínimo 1 e no máximo 100 aulas")
        @Valid
        List<CreateLessonRequest> lessons,
    @NotBlank(message = "ID do contribuidor é obrigatório") String contributorId) {
