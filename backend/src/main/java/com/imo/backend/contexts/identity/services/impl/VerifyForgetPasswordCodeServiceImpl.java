...
public class VerifyForgetPasswordCodeServiceImpl implements VerifyForgetPasswordCodeService {
  ...
  public VerifyForgetPasswordCodeServiceImpl(
      GetUserByIdGuard getUserByIdGuard, OutboxRepository<ForgetPasswordEvent> outboxRepository) {
        ...
        .anyMatch(
            outbox -> {
              ...
              this.outboxRepository.updateById(
                  outbox.getId(), Map.of("status", OutboxStatus.WAITING_TO_UPDATE_PASSWORD));
              return true;
            });
  }
}
