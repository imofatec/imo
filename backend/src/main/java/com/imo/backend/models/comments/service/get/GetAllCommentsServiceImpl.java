...
public class GetAllCommentsServiceImpl implements GetAllCommentsService {
  ...
  @Override
  public List<ConvertedCommentDto> execute(String lessonId) {
    String courseId = courseRepository.findByLessonId(lessonId).getId();
    return courseRepository.findCommentsByLessonId(courseId, lessonId).stream()
        .map(Comment::toDTO)
        .toList();
  }

  public List<ConvertedCommentDto> execute(String lessonId, Integer page, Integer size) {
    String courseId = courseRepository.findByLessonId(lessonId).getId();
    var pageable = Pageable.toMongodbAggregation(page, size);
    return courseRepository.findCommentsByLessonId(courseId, lessonId, pageable.get("skip"), pageable.get("limit")).stream()
        .map(Comment::toDTO)
        .toList();
  }
}
