...
@Service
public class GetAllCourseOverviewsByLevelWithPaginationService
        implements GetManyByToListWithPaginationService<CourseOverview> {
    private final CourseRepository courseRepository;

    public GetAllCourseOverviewsByLevelWithPaginationService(CourseRepository courseRepository) {
        this.courseRepository = courseRepository;
    }

    @Override
    public List<CourseOverview> execute(String slugLevel, Integer page, Integer size) {
        var pageable = Pageable.fromPageSize(page, size);

        return courseRepository.findAllBySlugLevel(slugLevel, pageable).stream()
