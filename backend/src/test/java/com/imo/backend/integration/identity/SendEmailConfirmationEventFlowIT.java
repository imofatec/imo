...
class SendEmailConfirmationEventFlowIT extends BaseMessagingTest {
  ...
  @Test
  @DisplayName("happy path: (fluxo completo): publicar SendEmailConfirmationEvent e verificar envio de email")
  void shouldSendEmailWhenSendEmailConfirmationEventIsPublished() {
    ...
    verify(mailManager, timeout(10_000)).sendWithTemplate(
        any(MailMessageBuilder.class),
        eq("confirmation_email"),
        eq(Map.of("name", name, "confirmationURL", "http://localhost:5173/user/confirmar-email"))
    );
  }
}
