...
public class E2EFlowHelper {
  ...
  public static CourseDetailsDTO createCourseAndReturnDetails(String token, int lessonsCount) {
    CreateCourseRequest courseRequest =
        CourseTestFactory.createValidCourseWithLessons(lessonsCount);
    ...
    try {
      ...
      throw new RuntimeException(
          "Erro ao desserializar CourseDetailsDTO: " + response.asString(), e);
    }
  }

  public static String markAllLessonsAsWatched(String token, CourseDetailsDTO courseDetails) {
    List<String> lessonIds =
        courseDetails.lessons().stream().map(lesson -> lesson.getId()).collect(Collectors.toList());
