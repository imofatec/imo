...
class LessonPoliciesTest {
  ...
  @Test
  @DisplayName("happy path (checkLessonConflicts): aceitar título, descrição e link únicos no curso")
  void shouldAllowUniqueLessonFieldsInCourse() {
    ...
    assertDoesNotThrow(() -> this.lessonPolicies.checkLessonConflicts(
        courseId,
        currentLessonId,
        newTitle,
        newLink,
  ...
  void shouldAllowUniqueLessonList() {
    assertDoesNotThrow(() -> this.lessonPolicies.checkListInternalConflicts(createLessonCommands()));
  }
  ...
  void shouldThrowWhenTitleIsDuplicated() {
    ...
    assertThrows(
        ...
        () -> this.lessonPolicies.checkLessonConflicts(courseId, currentLessonId, newTitle, newLink, newDescription)
    );
  ...
  void shouldThrowWhenTitlesConflictInsideList() {
    CreateLessonCommand lessonWithConflict = new CreateLessonCommand(
        "Introducao ao Java",
        "Primeiros passos com Java",
        "https://www.youtube.com/watch?v=abc123XYZ89"
    );
